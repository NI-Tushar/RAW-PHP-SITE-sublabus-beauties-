...
if (isset($_POST['submit']) && isset($_FILES['image-1']) && isset($_FILES['image-2'])){
    ...
    $image1path = 'products/glitters/'.$image1;
    $image2path = 'products/glitters/'.$image2;
...
if ($img_size1 > 1250000 && $img_size2 > 1250000) {
			...
			if (in_array($img_ex_lc1, $allowed_exs)) {
                 if(move_uploaded_file($tmp_name1, 'products/glitters/'.$image1))
                    {
            ...
            if (in_array($img_ex_lc2, $allowed_exs)) {
                 if(move_uploaded_file($tmp_name2, 'products/glitters/'.$image2))
                    {
...
if(!empty($id) || !empty($title) || !empty($model) || !empty($stock) || !empty($price)){
        ...
        if(mysqli_connect_error()){
            ...
            $sql = "INSERT INTO gliturs (id, title, model, stock, imgpath, price, altimgpath)
